...
const TRANSLATIONS = {
  ...
  malwareDetected: {
    ...
    de: "[pwned-check] MALWARE ERKANNT: pawns-cli l\xE4uft auf diesem System! Diese Software verkauft heimlich Ihre Internetbandbreite. Ihr ioBroker wurde wahrscheinlich \xFCber ein b\xF6sartiges Skript infiziert. Prozess beenden und globale Skripte sofort pr\xFCfen!",
    ...
    pt: "[pwned-check] MALWARE DETECTADO: pawns-cli est\xE1 a correr neste sistema! Este software vende secretamente a sua largura de banda. Termine o processo e verifique os scripts globais imediatamente!",
    nl: "[pwned-check] MALWARE GEDETECTEERD: pawns-cli draait op dit systeem! Deze software verkoopt uw internetbandbreedte. Be\xEBindig het proces en controleer uw globale scripts onmiddellijk!",
    fr: "[pwned-check] MALWARE D\xC9TECT\xC9 : pawns-cli tourne sur ce syst\xE8me ! Ce logiciel revend votre bande passante \xE0 votre insu. Arr\xEAtez le processus et v\xE9rifiez vos scripts globaux imm\xE9diatement !",
    it: "[pwned-check] MALWARE RILEVATO: pawns-cli \xE8 in esecuzione su questo sistema! Questo software vende segretamente la vostra larghezza di banda. Terminate il processo e controllate subito gli script globali!",
    es: "[pwned-check] MALWARE DETECTADO: \xA1pawns-cli est\xE1 corriendo en este sistema! Este software vende secretamente su ancho de banda. \xA1Detenga el proceso y revise sus scripts globales inmediatamente!",
    pl: "[pwned-check] WYKRYTO MALWARE: pawns-cli dzia\u0142a na tym systemie! To oprogramowanie potajemnie sprzedaje Tw\xF3j transfer internetowy. Zatrzymaj proces i natychmiast sprawd\u017A globalne skrypty!",
    ...
    "zh-cn": "[pwned-check] \u68C0\u6D4B\u5230\u6076\u610F\u8F6F\u4EF6\uFF1Apawns-cli \u6B63\u5728\u6B64\u7CFB\u7EDF\u4E0A\u8FD0\u884C\uFF01\u8BE5\u8F6F\u4EF6\u79D8\u5BC6\u51FA\u552E\u60A8\u7684\u7F51\u7EDC\u5E26\u5BBD\u3002\u8BF7\u7ACB\u5373\u7EC8\u6B62\u8BE5\u8FDB\u7A0B\u5E76\u68C0\u67E5\u5168\u5C40\u811A\u672C\uFF01"
  },
  emailCleared: {
  ...
  malwareTitle: {
    ...
    fr: "Malware pawns-cli d\xE9tect\xE9 !",
    it: "Rilevato malware pawns-cli!",
    es: "\xA1Detectado malware pawns-cli!",
  ...
  malwareSells: {
    ...
    fr: "Revend la bande passante et cr\xE9e des scripts globaux.",
  ...
  malwareAction: {
    ...
    de: "Prozess beenden, /tmp/pawns-cli l\xF6schen, globale Scripts pr\xFCfen!",
    ...
    nl: "Be\xEBindig het proces, verwijder /tmp/pawns-cli, controleer globale scripts!",
    fr: "Arr\xEAtez le processus, supprimez /tmp/pawns-cli, v\xE9rifiez les scripts globaux !",
    it: "Termina il processo, elimina /tmp/pawns-cli, controlla gli script globali!",
    es: "\xA1Detenga el proceso, borre /tmp/pawns-cli, revise los scripts globales!",
  ...
  malwareForum: {
    ...
    pt: "Informa\xE7\xF5es do f\xF3rum sobre o ataque",
    nl: "Foruminformatie over de aanval",
    fr: "Infos forum sur l'attaque",
    it: "Info forum sull'attacco",
    es: "Informaci\xF3n del foro sobre el ataque",
...
class PwnedCheck extends utils.Adapter {
   ...
   */
  async httpsGet(url, attempt = 1) {
    const controller = new AbortController();
    const timer = this.setTimeout(() => controller.abort(), 15e3);
    try {
  ...
  async updateVisualisation(config) {
    ...
    const malwareSection = malwareDetected ? (() => {
      ...
      const card = compactView ? `<div style="background:${safeCardBg};border:2px solid #e53935;border-radius:8px;padding:10px 14px;display:flex;align-items:center;gap:10px;">
						${warnSvg}
